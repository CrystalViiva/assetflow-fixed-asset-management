...
class AcquisitionPermission(BasePermission):
    """Keep acquisitions tenant-scoped and capitalization limited to managers."""

    message = "You do not have permission to manage asset acquisitions."
    READ_ROLES = {
        UserRole.ADMIN,
    ...
    def has_permission(self, request, view):
        ...
        if not user or not user.is_authenticated or not user.organization_id:
            return False
        if request.method in SAFE_METHODS:
            return user.is_superuser or user.role in self.READ_ROLES
        return user.is_superuser or user.role in ASSET_MANAGEMENT_ROLES
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in SAFE_METHODS:
            if request.user.role == UserRole.DEPARTMENT_MANAGER:
                return bool(request.user.department_id) and (
                    obj.asset.department_id == request.user.department_id
                    and obj.asset.department.organization_id == request.user.organization
                )
            return request.user.is_superuser or request.user.role in self.READ_ROLES
        return request.user.is_superuser or request.user.role in ASSET_MANAGEMENT_ROLES
